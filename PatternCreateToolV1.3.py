...
def PatternTemplate():
    TemplateWin = ttk.Toplevel(master=master,title="Pattern模板",size=(600,800),resizable=(False, False))
    TemplateWin.position_center()
    TemplateFrame = ttk.Labelframe(master=TemplateWin,text="I2CPattern模板")
    # TemplateMenu_ThemeMemu = ttk.Menu(master=GUIEdgeTopMenu)
    TemplateFrame.pack(fill=BOTH, expand=True,pady=8,padx=5)
    TemplateTable = TableCanvas(TemplateFrame,
                                cellwidth=60,
                                rowheight=34, rowheaderwidth=30,#如果rowheight 太小会导致entry里显示不全
                                editable=True)
    TemplateTable.show()
    TemplateTable.importCSV('test.csv')#导入外部的csv数据AutoFit
    # TemplateTable.load('test.table')#导入外部的.table数据AutoFit,tabel数据外部不可解析
    TemplateContrlFrame = ttk.Labelframe(master=TemplateWin, text="模板控制")
    TemplateContrlFrame.pack(fill=BOTH, expand=True,pady=8,padx=5)
    ttk.Button(master=TemplateContrlFrame,text="保存模板").grid(row=0,column=0,ipady=5,padx=10,pady=10)
    ttk.Button(master=TemplateContrlFrame, text="加载模板").grid(row=1,column=0,ipady=5,padx=10,pady=10)
